...
export default function VideoSliceKaleido({ videoTexture, segments=6, angle=0, scale=1, offsetX=0, offsetY=0, opacity=1, bpm=120, isGlobal=false, compositionWidth, compositionHeight }) {
  ...
  const fragmentShader = `
    uniform sampler2D tDiffuse; uniform vec2 resolution; uniform float uSegments; uniform float uAngle; uniform vec2 uOffset; uniform float uScale; uniform float uOpacity; uniform float uTime; uniform float uBpm; varying vec2 vUv; 
    const float TAU = 6.283185307179586; 
    void main(){ 
      vec2 uv = vUv - 0.5; 
      float r = length(uv); 
      float animatedAngle = uAngle;
      if (uBpm > 0.0) {
        float rotationSpeed = uBpm / 60.0;
        animatedAngle += uTime * rotationSpeed;
      }
      float theta = atan(uv.y, uv.x) + animatedAngle; 
  ...
  const shaderMaterial = useMemo(() => new THREE.ShaderMaterial({
    uniforms: {
      ...
      resolution: { value: new THREE.Vector2(effectiveW, effectiveH) }, uSegments: { value: Math.max(2, Math.floor(segments)) }, uAngle: { value: (angle * Math.PI)/180 }, uOffset: { value: new THREE.Vector2(offsetX, offsetY) }, uScale: { value: scale }, uOpacity: { value: opacity }, uTime: { value: 0 }, uBpm: { value: bpm },
  ...
  }), [videoTexture, blackTexture, isGlobal, renderTarget, effectiveW, effectiveH, segments, angle, offsetX, offsetY, scale, opacity, bpm]);
