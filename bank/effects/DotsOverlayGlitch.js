...
export default function DotsOverlayGlitch({ videoTexture, isGlobal=false, compositionWidth, compositionHeight, uDotSize=5.0, uGlitchIntensity=0.5, uOpacity=1.0 }){
  ...
  const geometry = useMemo(()=>{ const aspect = size?.width > 0 && size?.height > 0 ? size.width / size.height : 1920 / 1080; return new THREE.PlaneGeometry(aspect * 2, 2); }, [size?.width, size?.height]);
  ...
  return React.createElement('mesh',{ref:meshRef, geometry:geometry}, React.createElement('primitive',{object:shaderMaterial, ref:materialRef, attach:'material'}));
}
