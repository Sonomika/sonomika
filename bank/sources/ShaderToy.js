...
export const metadata = {
  name: 'ShaderToy', description: 'ShaderToy-like procedural shader source', category: 'Sources', author: 'AI', version: '1.0.0', isSource: true,
...
export default function ShaderToy({ shaderCode='', intensity=1.0, speed=1.0, color1='#ff0000', color2='#00ff00', color3='#0000ff', color4='#ffffff' }){
  if (!React || !THREE || !r3f) return null; const { useFrame, useThree } = r3f; const meshRef=useRef(null); const materialRef=useRef(null);
  ...
  useFrame((state)=>{ if (!materialRef.current) materialRef.current=shaderMaterial; if (!materialRef.current) return; materialRef.current.uniforms.iTime.value = state.clock.elapsedTime*speed; const w= size?.width||1920, h= size?.height||1080; materialRef.current.uniforms.iResolution.value.set(w,h,1); materialRef.current.uniforms.intensity.value=intensity; materialRef.current.uniforms.color1.value.set(color1); materialRef.current.uniforms.color2.value.set(color2); materialRef.current.uniforms.color3.value.set(color3); materialRef.current.uniforms.color4.value.set(color4); });
  return React.createElement('mesh',{ref:meshRef, position:[0,0,0]}, React.createElement('planeGeometry',{args:[aspect*2,2]}), React.createElement('primitive',{object:shaderMaterial, ref:materialRef}));
}
