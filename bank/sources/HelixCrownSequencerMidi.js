...
export const metadata = {
  ...
  parameters: [
    ...
    { name: 'scaleMode', type: 'number', value: 1, min: 0, max: 3, step: 1, description: '0 minor pentatonic, 1 dorian, 2 lydian, 3 whole tone' },
    ...
    { name: 'sendMidi', type: 'boolean', value: true, description: 'send MIDI notes to the selected MIDI output' },
...
function midiForLane(lane, rootMidi, scale) {
  const step = scale[lane % scale.length];
  const octaveOffset = Math.floor(lane / scale.length) * 12;
  ...
  useFrame((state, delta) => {
    ...
    if (currentStep !== lastStepRef.current) {
      ...
      for (let lane = 0; lane < laneCount; lane++) {
        ...
        const note = Math.max(0, Math.min(127, midiForLane(lane, rootMidi, scale)));
