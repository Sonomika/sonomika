// Portable external Bouncing Particle Cells Source (no imports). Uses globals: window.React, window.THREE, window.r3f
...
export const metadata = {
  name: 'Bouncing Particle Cells (External)',
...
export default function BouncingParticleCellsExternal({ numParticles=600, cellSize=0.02, speed=1.0, color='#66ccff', shape='square' }){
  ...
  React.useEffect(()=>{ const positions=new Array(numParticles); const velocities=new Array(numParticles); for (let i=0;i<numParticles;i++){ const x=(Math.random()*2-1)*(halfWidth-cellSize*0.5); const y=(Math.random()*2-1)*(halfHeight-cellSize*0.5); positions[i]=new THREE.Vector2(x,y); const ang=Math.random()*Math.PI*2; const base=.3+Math.random()*.7; const v=base*.5; velocities[i]=new THREE.Vector2(Math.cos(ang)*v, Math.sin(ang)*v); } positionsRef.current=positions; velocitiesRef.current=velocities; }, [numParticles, halfWidth, halfHeight, cellSize]);
  ...
  return React.createElement('instancedMesh', { ref: instancedRef, args:[geometry, material, numParticles], renderOrder: 9998 });
}
