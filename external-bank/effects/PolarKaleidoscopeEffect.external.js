// Portable external Polar Kaleidoscope effect (no imports). Uses globals: window.React, window.THREE, window.r3f
...
export const metadata = {
  name: 'Polar Kaleidoscope (External)',
...
export default function PolarKaleidoscopeExternal({ videoTexture, isGlobal = false, segments = 8, rotate = 0.2, swirl = 0.0, radialScale = 1.0, compositionWidth, compositionHeight }) {
  ...
  const fragmentShader = `
    uniform sampler2D tDiffuse; uniform vec2 uResolution; uniform float uTime; uniform float uSegments; uniform float uRotate; uniform float uSwirl; uniform float uRadialScale; varying vec2 vUv;
    void main(){ vec2 p = vUv*2.0 - 1.0; float r = length(p); float a = atan(p.y,p.x); float ang = a + uTime*uRotate + r*uSwirl; float segAng = 6.28318530718 / max(1.0, uSegments); ang = mod(ang, segAng); ang = abs(ang - segAng*0.5); vec2 q = vec2(cos(ang), sin(ang)) * r * uRadialScale; vec2 uv = (q + 1.0)*0.5; vec3 col = texture2D(tDiffuse, uv).rgb; gl_FragColor = vec4(col,1.0); }
  `;
  ...
  const shaderMaterial = useMemo(() => new THREE.ShaderMaterial({
    uniforms: {
      ...
      uResolution: { value: new THREE.Vector2(effectiveW, effectiveH) }, uTime: { value: 0 }, uSegments: { value: segments }, uRotate: { value: rotate }, uSwirl: { value: swirl }, uRadialScale: { value: radialScale },
    }, vertexShader, fragmentShader, transparent: true, depthTest: false, depthWrite: false,
  }), [videoTexture, isGlobal, renderTarget, effectiveW, effectiveH, segments, rotate, swirl, radialScale]);
